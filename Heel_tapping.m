...
% Version :  1.
% Function:  Heel_tapping GUI
% Author  :  Dr. Safwan Al-Qadhi

function varargout = Heel_tapping(varargin)
%HEEL_TAPPING M-file for Heel_tapping.fig
%      HEEL_TAPPING, by itself, creates a new HEEL_TAPPING or raises the existing
%      singleton*.
%
%      H = HEEL_TAPPING returns the handle to a new HEEL_TAPPING or the handle to
%      the existing singleton*.
%
%      HEEL_TAPPING('Property','Value',...) creates a new HEEL_TAPPING using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to Heel_tapping_OpeningFcn.  This calling syntax produces a
%
%      HEEL_TAPPING('CALLBACK') and HEEL_TAPPING('CALLBACK',hObject,...) call the
%      local function named CALLBACK in HEEL_TAPPING.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help Heel_tapping

% Last Modified by GUIDE v2.5 21-Oct-2008 23:34:01
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Heel_tapping_OpeningFcn, ...
                   'gui_OutputFcn',  @Heel_tapping_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
%% --- Executes just before Heel_tapping is made visible.
function Heel_tapping_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for Heel_tapping
...
% UIWAIT makes Heel_tapping wait for user response (see UIRESUME)
% uiwait(handles.figure1);

%% --- Outputs from this function are returned to the command line.
function varargout = Heel_tapping_OutputFcn(hObject, eventdata, handles)
...
fclose('all');
disp(' ');
disp('***** END Heel Tapping Analysis GUI *****');
disp(' ');
disp(' ');
clear all;
