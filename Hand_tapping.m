...
% Version :  1.
% Function:  Hand_tapping GUI
% Author  :  Dr. Safwan Al-Qadhi

function varargout = Hand_tapping(varargin)
%HAND_TAPPING M-file for Hand_tapping.fig
%      HAND_TAPPING, by itself, creates a new HAND_TAPPING or raises the existing
%      singleton*.
%
%      H = HAND_TAPPING returns the handle to a new HAND_TAPPING or the handle to
%      the existing singleton*.
%
%      HAND_TAPPING('Property','Value',...) creates a new HAND_TAPPING using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to Hand_tapping_OpeningFcn.  This calling syntax produces a
%
%      HAND_TAPPING('CALLBACK') and HAND_TAPPING('CALLBACK',hObject,...) call the
%      local function named CALLBACK in HAND_TAPPING.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help Hand_tapping

% Last Modified by GUIDE v2.5 21-Oct-2008 23:31:25
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Hand_tapping_OpeningFcn, ...
                   'gui_OutputFcn',  @Hand_tapping_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
%% --- Executes just before Hand_tapping is made visible.
function Hand_tapping_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for Hand_tapping
...
% UIWAIT makes Hand_tapping wait for user response (see UIRESUME)
% uiwait(handles.figure1);

%% --- Outputs from this function are returned to the command line.
function varargout = Hand_tapping_OutputFcn(hObject, eventdata, handles)
...
fclose('all');
disp(' ');
disp('***** END Hand Tapping Analysis GUI *****');
disp(' ');
disp(' ');
clear all;
