...
% Version :  1.
% Function:  Gait_Analysis_Left GUI
% Author  :  Dr. Safwan Al-Qadhi

function varargout = Gait_Analysis_Left(varargin)
%GAIT_ANALYSIS_LEFT M-file for Gait_Analysis_Left.fig
%      GAIT_ANALYSIS_LEFT, by itself, creates a new GAIT_ANALYSIS_LEFT or raises the existing
%      singleton*.
%
%      H = GAIT_ANALYSIS_LEFT returns the handle to a new GAIT_ANALYSIS_LEFT or the handle to
%      the existing singleton*.
%
%      GAIT_ANALYSIS_LEFT('Property','Value',...) creates a new GAIT_ANALYSIS_LEFT using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to Gait_Analysis_Left_OpeningFcn.  This calling syntax produces a
%
%      GAIT_ANALYSIS_LEFT('CALLBACK') and GAIT_ANALYSIS_LEFT('CALLBACK',hObject,...) call the
%      local function named CALLBACK in GAIT_ANALYSIS_LEFT.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help Gait_Analysis_Left

% Last Modified by GUIDE v2.5 21-Oct-2008 23:19:15
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Gait_Analysis_Left_OpeningFcn, ...
                   'gui_OutputFcn',  @Gait_Analysis_Left_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
%% --- Executes just before Gait_Analysis_Left is made visible.
function Gait_Analysis_Left_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for Gait_Analysis_Left
...
% UIWAIT makes Gait_Analysis_Left wait for user response (see UIRESUME)
% uiwait(handles.figure1);

%% --- Outputs from this function are returned to the command line.
function varargout = Gait_Analysis_Left_OutputFcn(hObject, eventdata, handles)
...
frame = getframe(Gait_Analysis_Left);
...
%         close force
    ...
    fclose('all');
    disp(' ');
    disp('***** END Gait Analysis Left GUI *****');
    disp(' ');
    disp(' ');
    clear all;
